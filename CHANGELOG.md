...
- **A natives page built from the includes.** [`docs/natives.md`](https://nullsablex.github.io/mysql_samp/natives/) lists every native, the forward and the documented constants, each with its signature, its open.mp alias, its parameters and its return — generated by `scripts/gen_natives_page.py` on every docs build rather than written by hand, so it cannot drift from what the plugin declares. Two comment formats are read: JavaDoc, which the includes here use, and pawndoc (`<summary>`, `<param name="">`, `<returns>`), which a project is likely to meet in includes it pulls in. A `--selftest` covers both against fixtures, and `--check` fails the docs build if an entry has no documentation block.
...
- **`rust/cleartext-logging` alerts from CodeQL, closed at the actual sink.** These are false positives — the password never reaches a log in any version — but they were fixed in code rather than dismissed (dismissal is repo state that decays as lines move). It took three passes, because the first two aimed at the wrong place. #33 replaced the free-form failure `String` with a fixed `PasswordFailure` enum; #36 stopped two infrastructure `Logger::error` calls from interpolating the callback name. Four alerts survived both, and reading the SARIF code flow explains why: the reported sink was never a log call at all. It is `info.params` in `process_pending_passwords`. CodeQL's generated model for the `smallvec` crate marks `<alloc::vec::Vec ...>::insert` as a `log-injection` sink, so every `params.insert(0, ..)` fed by a value reached from `self.passwords.poll_results()` — flagged sensitive by the `password` name heuristic, though what flows is an Argon2id PHC hash or a `bool` — raised one alert. Four inserts, four alerts. The callback's first argument is now built once and the parameter vector rebuilt with `push` + `append` instead of `insert(0, ..)`, which is what `insert` at index 0 does internally anyway; behaviour is unchanged. Copilot Autofix has nothing to offer on these: asked for a suggestion it returns `error` / `no suggested fix found`, because the fix it knows how to write is redacting a log call and the flagged expression is a `Vec` insert. (#33, #36, #37)
- **Releases now ship both includes.** `mysql_samp_omp.inc` was added in #34 but the release workflow still staged and uploaded only `mysql_samp.inc`, so the open.mp-styled include existed in the repository and in no release. Both are staged, listed in the release body and attached to the tag. (#37)
...
- **Encoding guard for the Pawn sources.** The SA-MP and open.mp compilers read `.inc` / `.pwn` files as bytes, not UTF-8, so a stray accented character or typographic dash reaches the server as mojibake instead of failing loudly at compile time — the cleanup in #35 fixed the files, this keeps them fixed. The rules: pure ASCII, no UTF-8 BOM, no stray control bytes (CR, NUL — tab and LF allowed), and the generated `mysql_samp.inc` still matching its template with the current version substituted. They run in two places, with different scopes on purpose. The `Encoding` workflow (`.github/workflows/encoding.yml` → `.github/scripts/check_pawn_encoding.py`, standard library only, `permissions: {}` at the top level with `contents: read` on the single job) covers both includes plus **everything** under `examples/` and `benchmark/` — not only the `.pwn` scripts but `schema.sql`, `setup.sql` and `mysql.ini.example`, which the plugin itself reads through `mysql_query_file` and `mysql_connect_file`; the listing is opt-out (`.md` only), so a new fixture is covered the day it is added. `tests/inc_encoding.rs` covers only the three includes, which are build output of the crate, so that editing an example never hinges on the Rust suite. (#37)
- **Documentation covers both includes.** README, `docs/index.md`, `docs/installation.md`, `docs/api-reference.md`, `examples/README.md` and `CHECKLIST.md` were written when there was a single include. They now state the choice explicitly — `<mysql_samp>` for snake_case, `<mysql_samp_omp>` for `Prefix_PascalCase`, alternatives rather than layers, one per script — and the API reference notes that its snake_case listing describes both. (#37)
- **Dropped the GitHub Pages custom domain.** `docs/CNAME` is gone and `site_url` in `mkdocs.yml` points at `https://nullsablex.github.io/mysql_samp/`, so the published site and its canonical URLs agree. (#37)
...
- **`MYSQL_OPT_SSL` never encrypted anything.** The `mysql` crate was built with the `default-rust` feature, which — despite the name — enables no TLS backend at all. Without one the crate's `make_secure` is a `panic!` stub, so enabling SSL aborted the connection rather than securing it. v1.1.1 announced these options as wired through "(rustls)"; that was wrong. **If you enabled `MYSQL_OPT_SSL` on an older build, your traffic was never encrypted** — treat those credentials as exposed.

  `rustls-tls-ring` is now enabled and TLS was verified end to end against MariaDB 11.8: the server's completed-handshake counter advances for each connection, and with the default `MYSQL_OPT_SSL_VERIFY_CERT` a self-signed certificate is rejected with `invalid peer certificate: UnknownIssuer` rather than silently falling back.

  Note that TLS is not available over a unix socket — a host starting with `/` has nothing to encrypt. Use a TCP host if you need it.

- **SQL injection under `sql_mode=NO_BACKSLASH_ESCAPES`.** `mysql_escape_string`, `mysql_format` and the ORM escaped `'` as `\'`. Under that mode the backslash is not an escape character, so `\'` is a literal backslash followed by a *live* quote and a crafted value breaks out of the literal. Because the driver always enables `CLIENT_MULTI_STATEMENTS` (the crate forbids disabling it), that escape is enough to append a second statement. The escaping mode is now detected per connection from the server's `sql_mode` and switches to quote doubling, matching `mysql_real_escape_string`. Verified live against MariaDB with `sql_mode=NO_BACKSLASH_ESCAPES`: the plugin logs the mode switch, `mysql_escape_string("O'Brien")` yields `O''Brien`, and an injection payload aimed at that mode stays contained inside the string literal.
...
- **Prepared statements returned nothing for any numeric column.** `Row::get::<Option<String>>` goes through `from_value`, which **panics** on a failed conversion. Over the text protocol every value arrives as bytes, so it never fired — but a prepared statement uses the binary protocol, where an `INT` arrives as `Value::Int` and `String` does not accept it. The worker thread died silently: no rows, no callback, no error, no log line. Any `mysql_stmt_execute` selecting a number was affected, which is most of them. Values are now converted explicitly, rendering numbers the way the text protocol does so a column reads the same either way. Found by running against a real server; no unit test could have caught it.
