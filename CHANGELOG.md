...
- **`MYSQL_OPT_SSL` never encrypted anything, and now fails loudly instead of silently.** The `mysql` crate was built with the `default-rust` feature, which — despite the name — enables no TLS backend at all. Without one the crate's `make_secure` is a `panic!` stub, so enabling SSL aborted the connection rather than securing it. v1.1.1 announced these options as wired through "(rustls)"; that was wrong. **If you enabled `MYSQL_OPT_SSL` on an older build, your traffic was never encrypted** — treat those credentials as exposed.

  `rustls-tls-ring` is now enabled, but live testing against MariaDB 11.8 showed the driver can still complete the handshake **in clear text without reporting an error**, which is a worse failure than the panic it replaced. So the plugin no longer trusts the driver: after connecting with `MYSQL_OPT_SSL`, it asks the server for `Ssl_cipher` and **refuses the connection** when the session is not actually encrypted, rather than sending credentials in the clear. When TLS does come up, the negotiated cipher is logged.

  Two consequences worth knowing: TLS is not available over a unix socket (use a TCP host), and a `MYSQL_OPT_SSL` connection that used to "work" may now be refused — that refusal is the bug becoming visible, not a regression.
