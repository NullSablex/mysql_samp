...
- **Encoding guard for the Pawn sources.** The SA-MP and open.mp compilers read `.inc` / `.pwn` files as bytes, not UTF-8, so a stray accented character or typographic dash reaches the server as mojibake instead of failing loudly at compile time — the cleanup in #35 fixed the files, this keeps them fixed. `include/mysql_samp.inc.in`, both generated includes and every `examples/` and `benchmark/` script are checked for pure ASCII, no UTF-8 BOM, no stray control bytes (CR, NUL — tab and LF allowed), and that the generated `mysql_samp.inc` still matches its template with the current version substituted. The same rules run in two places: the `Encoding` workflow (`.github/workflows/encoding.yml` → `.github/scripts/check_pawn_encoding.py`, standard library only, `permissions: {}` at the top level with `contents: read` on the single job) so a pull request fails with inline annotations, and `tests/inc_encoding.rs` so `cargo test` catches it before a push. (#37)
