...
Full table of every native and forward exposed by `mysql_samp`. The plugin registers **79 natives** and **1 forward**.
...
Every name below is listed in its snake_case form, as declared by `<mysql_samp>`. The alternative include `<mysql_samp_omp>` declares the exact same set under open.mp's `Prefix_PascalCase` convention — `mysql_connect` becomes `MySQL_Connect`, `cache_get_row_count` becomes `Cache_GetRowCount`, `orm_create` becomes `ORM_Create`. It is generated from the same template at build time and is aliases only, so this page describes both: translate the name, not the behaviour. See [Installation](installation.md#place-the-include).
...
The literal is regenerated by `build.rs` from `CARGO_PKG_VERSION` on every build, so the value shipped in both includes always tracks the version declared in [`Cargo.toml`](https://github.com/NullSablex/mysql_samp/blob/master/Cargo.toml). This page does not hardcode the current number on purpose — check `Cargo.toml` or the include itself.

```pawn
#define MYSQL_SYNC "!sync"
```

Written where a callback name goes, it makes that one call blocking — see
[Queries](queries.md#mysql_sync--making-one-call-block-on-purpose). Accepted by
`mysql_query`, `mysql_pquery` and `mysql_query_file`; every other native that
takes a callback refuses it out loud rather than mistaking it for one.
...
| `mysql_tls_active(connId)` | `bool` | Whether the session is encrypted, as the handshake settled it |
| `mysql_tls_cipher(connId, dest[], max_len = sizeof(dest))` | `bool` | Writes the negotiated cipher suite; empty when unencrypted |
| `mysql_limit_set(limit, value)` | `bool` | Raises or lowers a memory cap (`MYSQL_LIMIT_*`) |
| `mysql_limit_get(limit)` | `int` | Reads a memory cap |
